...
if __name__ == '__main__':
    ...
    doc_path_list = []
    for pdf_files_dir in os.listdir(pdf_base_dir):
        pdf_files_dir = os.path.join(pdf_base_dir,pdf_files_dir)
        for doc_path in Path(pdf_files_dir).glob('*'):
            if guess_suffix_by_path(doc_path) in pdf_suffixes + image_suffixes:
                doc_path_list.append(doc_path)

    print(f'pdf files count: {len(doc_path_list)}')
    print(doc_path_list[0])
